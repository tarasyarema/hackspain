...
def main():
    ...
    parser.add_argument("--preserve-face", choices=("top", "bottom"), default="top")
    parser.add_argument("--route", choices=("keep", "reject"), default="reject")
    ...
    shift_sign = -1 if args.preserve_face == "top" else 1
    proposed_center = center + shift_sign * normal * (args.half_thickness - .002)
    ...
    result = {
        ...
        "preserved_face": args.preserve_face,
        "route": args.route,
        "preserved_face_center_m": (proposed_center - shift_sign * normal * args.half_thickness).tolist(),
        "reject_clearance": [],
    }
    for path in sorted(args.traces.glob(f"{args.route}-seed-*-reuse-*.json.gz")):
    ...
    if len(result["reject_clearance"]) != 8:
        raise ValueError("expected all eight stored routes")
