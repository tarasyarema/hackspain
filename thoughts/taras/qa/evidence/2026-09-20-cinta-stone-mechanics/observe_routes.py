...
def differences(observed, expected, path=''):
    if isinstance(observed, dict) and isinstance(expected, dict):
        ...
        for key in sorted(observed.keys() | expected.keys()):
            ...
            if key not in observed or key not in expected:
                result.append({'path': child, 'observed': observed.get(key), 'expected': expected.get(key)})
            else:
                result.extend(differences(observed[key], expected[key], child))
        return result
    return [] if observed == expected else [{'path': path, 'observed': observed, 'expected': expected}]
...
def summarize(row):
    ...
    return {'seed': row['seed'], 'reuse': row['reuse'], 'mode': row['mode'],
            ...
            'reference_differences': row['reference_differences'],
            'all_non_wall_fields_match_reference': row['all_non_wall_fields_match_reference'],
...
def main():
                ...
                with tempfile.TemporaryDirectory() as directory:
                    ...
                    try:
                        ...
                        for reuse in range(2):
                            ...
                            row = {'mode': mode, 'seed': seed, 'reuse': reuse, 'object_id': int(uid),
                                   'body_id': int(bean.body), 'mass_kg': float(bean.mass), 'axes_m': bean.axes.tolist(),
                                   'trajectory': [], 'contacts': []}
                            ...
                            row['reference_differences'] = differences(row['original'], expected)
                            row['all_non_wall_fields_match_reference'] = all(
                                d['path'] == 'decision.t_available_s' for d in row['reference_differences'])
                            output = OUT / f'{mode}-seed-{seed}-reuse-{reuse}.json.gz'
                            with gzip.open(output, 'wt') as stream:
                                json.dump(row, stream, allow_nan=False)
                            comparison = {'observed': row['original'], 'expected': expected,
                                          'differences': row['reference_differences']}
                            (OUT / f'{mode}-seed-{seed}-reuse-{reuse}-comparison.json').write_text(
                                json.dumps(comparison, indent=2, allow_nan=False) + '\n')
                            assert row['all_non_wall_fields_match_reference'], (mode, seed, reuse, row['reference_differences'])
                            if mode == 'reject':
                                for original in (row['original'], expected):
                                    decision = original['decision']
                                    assert decision['t_available_s'] < decision['t_fire_s'] - .0015
    ...
    result = {'source_revision': revision, 'model_sha256': EXPECTED_MODEL,
              ...
              'comparison_exception': 'decision.t_available_s only, with unchanged valve window',
