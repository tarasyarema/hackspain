    ...
    with tempfile.TemporaryDirectory(prefix="cinta-overdue-") as temp_dir:
        ...
        try:
            ...
            def remember_initial(bean):
                ...
                if bean.cls != row["expected_class"]:
                    result["errors"].append(
                        f"target {bean.uid} sampled class {bean.cls!r}, expected {row['expected_class']!r}"
                    )
            ...
            if not snapshot_done:
                result["errors"].append("4.0 second endpoint was not reached")
            for uid, row in capture.items():
                if "sample" not in row:
                    result["errors"].append(f"target {uid} was not observed by 5.5 seconds")
