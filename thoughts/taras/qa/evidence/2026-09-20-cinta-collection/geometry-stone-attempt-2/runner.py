...
def install_observers(sim, capture: dict, observe_support_enabled: bool) -> None:
    ...
    def observed_step():
        capture["pre_step_states"] = {
            body: state(sim, sim.data, body) for body in sim.bean_of
        }
    ...
    def observed_park(body: int):
        ...
        if bean is not None:
            ...
            capture["parks"].append({
                ...
                "contacts": contacts,
                "state_before_contact_step": capture["pre_step_states"][body],
                "contact_solve_pose": list(bean.last_pos) + list(bean.last_quat),
    ...
    if args.mode == "stone":
        validation = {
            ...
            "expected_route_matches": sum(
                row["collection"]["outcome"] == row["expected_outcome"] for row in rows
            ),
            "single_outcome_and_retirement": sum(
                row["collection"]["outcome_event_count"] == 1
                and row["collection"]["retirement_event_count"] == 1 for row in rows
            ),
        }
        if args.observe_support:
            validation["support_matches"] = 0
            for row in rows:
                observation = row["collection"].get("support_observation") or {}
                terminal = observation.get("final_100ms") or {}
                support = terminal.get("support", {}).get(row["expected_outcome"], {})
                validation["support_matches"] += bool(support.get("stable", False))
        validation["passed"] = all(value == 16 for value in validation.values())
        summary["collection_validation"] = validation
